...
export function SettingsOverview({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const evoRes = await fetch('/api/evolution/config', { cache: 'no-store' });
        if (!cancelled && evoRes.ok) {
          const data = await evoRes.json();
          setEvolution({
            configured: !!data.configured,
            connected: data?.config?.status === 'connected',
          });
        } else if (!cancelled) {
          setEvolution({ configured: false, connected: false });
        }
      } catch {
        if (!cancelled) setEvolution({ configured: false, connected: false });
      } finally {
